...
**Done.** Steps 1 to 6 are built and tested (`tests/test_web.py`,
`music_tools/web/`, `practice serve`). Step 7, the cutover, was taken in one
move rather than over a fortnight: the whole history is imported, and practice
is driven from the page from here on. What that skips is written up under
"Cutover" below.
...
Not code. A checklist, and what actually happened:

1. ~~Export every sheet, import for real~~ — done, with the module named in the
   argument (`--modules 'SONGS:<path>'`) because Google names a single-sheet
   export after the document.
2. ~~Practise from both for a week, the sheet authoritative~~ — **skipped, by
   choice.** The parallel week was insurance against a bad port; the importer
   is tested against the real export, is idempotent, and re-importing is
   cheap, so the sheet is retired now instead.
3. ~~Compare day totals each evening~~ — folded into the above. The sheet's own
   `2026-07-05` subtotals (00:19 / 00:34 / 00:53) stay pinned as a test.
4. Fill in `target_bpm` for the rows the module view flags, a few at a time.
   **Ongoing**, and by design: the flag is on the page, so it gets closed by
   use rather than as a migration chore.
5. ~~Stop opening the sheet~~ — done. `docs/raw/` stays as the record of what
   was replaced.
6. Run `task db:dump` and commit `backups/practice.sql` — **outstanding, and
   the one loose end worth minding.** The spreadsheet gave version history for
   free; a SQLite file gives none, so until the dump is a habit an accident is
   unrecoverable.
...
Editing the day log's past days is not in this phase — not for want of trust,
which was the reason given here first and was the wrong one. This is a
practice log for one player, not a ledger: the worst a bad correction does is
misreport how long somebody spent on a tune, and the player who made it is the
only one reading it. It waited because the phase was already the cutover, and
it turned up immediately afterwards.
