...
(`catalogue.move_exercises`, `POST /modules/{slug}/move`,
`_queue_rows.html`, `practice move`, and the tick column on
`_exercise_row.html`). Two notes on what the build decided:
