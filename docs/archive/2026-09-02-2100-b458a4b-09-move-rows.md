...
**Done.** All five steps are built and tested
(`catalogue.move_exercises`, `POST /modules/{slug}/move`, `practice move`,
and the tick column on `_exercise_row.html`). Three notes on what the build
decided:

- **The address moved from `POST /exercises/move` to
  `POST /modules/{slug}/move`** — see the decision below. The first shape was
  written, and every move came back 422: `/exercises/{exercise_id}` is declared
  above it and read `move` as an id. The module the rows leave belongs in the
  path anyway, since it is the page that answers.
- **`form="move"` was checked in a real browser**, not only through the test
  client, because the whole tick-box design rests on it: with HTMX the POST
  body is `exercise_id=1&exercise_id=3&module_id=1`, and with JavaScript
  disabled the plain submit moves the row and redirects back to the page.
- **The planned `_queue_rows.html` is not needed: `_queue.html` already
  exists.** #31 landed the same fragment first, for the same reason from the
  other direction — start and stop move a due date, so the row has moved in the
  order and swapping it alone left it where it was. It carries the `<tbody>`
  itself, so a move answers with it and swaps `outerHTML`, and the empty-queue
  line stays the `<p>` outside the table that #31 kept.
...
- **The answer to a move is the whole queue, not a row.** Every other write in
  ...
  the fragment is the tbody — `_queue.html`, targeted `outerHTML`.
...
music_tools/
    ...
    web/templates/_queue.html           the tbody #31 had already extracted
...
3. **The page.** The tick column in `_queue_head.html` and `_exercise_row.html`,
   the move bar in `module.html` — hidden when there is nowhere to move to —
   the move response rendering the same `_queue.html` the page does, and the
   CSS.
